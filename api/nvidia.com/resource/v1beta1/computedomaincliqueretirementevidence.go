...
// +kubebuilder:validation:XValidation:rule="self.reason != 'NodeReboot' || (self.activationBootID.size() > 0 && self.witnessBootID.size() > 0 && self.activationBootID != self.witnessBootID)",message="node-reboot evidence requires distinct nonempty boot IDs"
