...
func TestComputeDomainCliqueProtocolCompatibility(t *testing.T) {
	...
	daemon.Mode = ComputeDomainDaemonModePersistentAgent
	daemon.Protocol = ComputeDomainCliqueProtocolPersistentAgentV1
	require.NoError(t, daemon.Validate())

	channel.Protocol = ComputeDomainCliqueProtocolPersistentAgentV1
	require.NoError(t, channel.Validate())
	require.Equal(t, ComputeDomainCliqueProtocolPersistentAgentV1, EffectiveComputeDomainCliqueSnapshotProtocol(""))
...
func TestComputeDomainDaemonModeCompatibility(t *testing.T) {
	...
	t.Run("persistent agent has no synthetic domain", func(t *testing.T) {
		...
		config.Protocol = ComputeDomainCliqueProtocolPersistentAgentV1
		require.NoError(t, config.Normalize())
		require.NoError(t, config.Validate())
	})
	...
	for name, config := range map[string]*ComputeDomainDaemonConfig{
		...
		"persistent protocol in per-domain mode": {
			Mode:     ComputeDomainDaemonModePerDomain,
			DomainID: "domain",
			Protocol: ComputeDomainCliqueProtocolPersistentAgentV1,
		},
		"legacy protocol in persistent mode": {
			Mode:     ComputeDomainDaemonModePersistentAgent,
			Protocol: ComputeDomainCliqueProtocolLegacyV1,
		},
		"unknown mode": {
			Mode: "Other",
		},
