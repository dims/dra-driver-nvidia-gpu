...
const (
	...
	ComputeDomainCliqueReservationReleaseReasonProcessExit    = "VerifiedProcessExit"
)
...
// +kubebuilder:validation:XValidation:rule="!has(self.status) || !has(self.status.phase) || self.status.phase != 'Released' || (has(self.status.releasedAt) && self.status.releaseReason in ['NeverPublished', 'VerifiedProcessExit'])",message="Released reservation status requires typed release evidence and timestamp"
// +kubebuilder:validation:XValidation:rule="!has(self.status) || !has(self.status.phase) || self.status.phase != 'Released' || self.status.releaseReason != 'VerifiedProcessExit' || (self.status.snapshotUID != '' && self.status.activationGeneration > 0 && self.status.activationHash.size() == 64 && self.status.fencedGeneration >= self.status.activationGeneration && self.status.fencedHash.size() == 64)",message="verified process-exit release requires exact activation and fenced snapshot identities"
...
// that every exact published daemon acknowledged supervised IMEX process exit.
...
type ComputeDomainCliqueReservationStatus struct {
	...
	// +kubebuilder:validation:Enum=NeverPublished;VerifiedProcessExit
	...
	// for which every exact daemon supplied process-exit evidence.
	// +kubebuilder:validation:Minimum=0
