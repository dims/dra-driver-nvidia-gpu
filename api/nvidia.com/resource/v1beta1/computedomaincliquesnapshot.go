...
// ComputeDomainCliqueSnapshot is the allocation and current membership
// snapshot for one hardware clique in one ComputeDomain. Persistent agents are
...
// frequently changing data lives in the status subresource.
...
type ComputeDomainCliqueSnapshotSpec struct {
	...
	// Protocol identifies the only daemon provider which consumes snapshots.
	// +kubebuilder:validation:Enum=persistent-agent-v1
	Protocol ComputeDomainCliqueProtocol `json:"protocol"`
}

// EffectiveComputeDomainCliqueSnapshotProtocol returns the snapshot provider.
// Empty is accepted only so an incomplete create can be rejected uniformly by
// runtime validation; the CRD requires the field.
func EffectiveComputeDomainCliqueSnapshotProtocol(protocol ComputeDomainCliqueProtocol) ComputeDomainCliqueProtocol {
	if protocol == "" {
		return ComputeDomainCliqueProtocolPersistentAgentV1
	}
	return protocol
}
