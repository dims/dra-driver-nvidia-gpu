...
func newNodeAttestationFixture(t *testing.T, suffix string, protocol nvapi.ComputeDomainCliqueProtocol) *nodeAttestationFixture {
	...
	templateChannel := channel.DeepCopy()
	...
	require.NoError(t, manager.nodeInformer.GetStore().Add(node.DeepCopy()))
...
func TestNodeAttestationAcquiresReservationBeforePublishingPersistentAgentRoute(t *testing.T) {
	f := newNodeAttestationFixture(t, "controller", nvapi.ComputeDomainCliqueProtocolPersistentAgentV1)
...
func TestNodeAttestationReportsMissingWholeCliqueIsolationOnce(t *testing.T) {
	f := newNodeAttestationFixture(t, "isolation-event", nvapi.ComputeDomainCliqueProtocolPersistentAgentV1)
	node := f.node.DeepCopy()
	delete(node.Labels, persistentAgentIsolationLabelKey)
	f.core.putNode(node)
	require.NoError(t, f.manager.nodeInformer.GetStore().Update(node.DeepCopy()))
	...
	require.Contains(t, messages[0], persistentAgentIsolationLabelKey+"="+string(f.cd.UID))
	...
	_, err = f.nvidia.ResourceV1beta1().ComputeDomainCliqueReservations().Get(context.Background(), cdclique.ReservationName("physical-clique"), metav1.GetOptions{})
...
func TestLiveNodeAttestationRejectsStaleClaimAfterRestart(t *testing.T) {
	f := newNodeAttestationFixture(t, "stale-claim", nvapi.ComputeDomainCliqueProtocolPersistentAgentV1)
	require.NoError(t, f.manager.reconcileNodeAttestation(context.Background(), f.node.Name))
	node, err := f.core.CoreV1().Nodes().Get(context.Background(), f.node.Name, metav1.GetOptions{})
	require.NoError(t, err)
	require.NoError(t, f.manager.nodeInformer.GetStore().Update(node.DeepCopy()))
...
func TestPublishedSnapshotKeepsDaemonRouteAfterWorkloadDeletion(t *testing.T) {
	f := newNodeAttestationFixture(t, "retirement-route", nvapi.ComputeDomainCliqueProtocolPersistentAgentV1)
	require.NoError(t, f.manager.reconcileNodeAttestation(context.Background(), f.node.Name))
	node, err := f.core.CoreV1().Nodes().Get(context.Background(), f.node.Name, metav1.GetOptions{})
	require.NoError(t, err)
	require.NoError(t, f.manager.nodeInformer.GetStore().Update(node.DeepCopy()))
	...
	require.True(t, validNodeAttestation(after))
}

func TestReservationCreateIsSerializedPerPhysicalClique(t *testing.T) {
	f := newNodeAttestationFixture(t, "singleflight", nvapi.ComputeDomainCliqueProtocolPersistentAgentV1)
	start := make(chan struct{})
	errs := make(chan error, 2)
	for range 2 {
		go func() {
			<-start
			errs <- f.manager.reservePhysicalClique(context.Background(), f.cd, "physical-clique")
		}()
	}
	close(start)
	for range 2 {
...
func TestIsolationBoundaryChangeRequeuesWholePhysicalClique(t *testing.T) {
	f := newNodeAttestationFixture(t, "fanout", nvapi.ComputeDomainCliqueProtocolPersistentAgentV1)
	second := f.node.DeepCopy()
	second.Name = "node-fanout-second"
	second.UID = "node-uid-fanout-second"
	require.NoError(t, f.manager.nodeInformer.GetStore().Add(second))

	previous := f.node.DeepCopy()
	delete(previous.Labels, persistentAgentIsolationLabelKey)
...
func TestStartupCliqueIdentityChangeRequeuesWholePhysicalClique(t *testing.T) {
	f := newNodeAttestationFixture(t, "startup-fanout", nvapi.ComputeDomainCliqueProtocolPersistentAgentV1)
	...
	require.NoError(t, f.manager.nodeInformer.GetStore().Add(peer))
...
func TestNodeAttestationDoesNotTakeOverLegacyRouting(t *testing.T) {
	...
	delete(f.node.Labels, persistentAgentIsolationLabelKey)
	require.NoError(t, f.manager.nodeInformer.GetStore().Update(f.node.DeepCopy()))
...
func TestNodeAttestationRequiresCliqueAndIsolation(t *testing.T) {
	for name, label := range map[string]string{
		"physical clique": gpuCliqueNodeLabelKey,
		"isolation":       persistentAgentIsolationLabelKey,
	} {
		t.Run(name, func(t *testing.T) {
			f := newNodeAttestationFixture(t, "missing-label", nvapi.ComputeDomainCliqueProtocolPersistentAgentV1)
			delete(f.node.Labels, label)
			require.NoError(t, f.manager.nodeInformer.GetStore().Update(f.node.DeepCopy()))
			require.NoError(t, f.manager.reconcileNodeAttestation(context.Background(), f.node.Name))
			node, err := f.core.CoreV1().Nodes().Get(context.Background(), f.node.Name, metav1.GetOptions{})
			require.NoError(t, err)
...
func TestNodeAttestationRejectsIsolatedCliqueWithExistingLegacyRoute(t *testing.T) {
	f := newNodeAttestationFixture(t, "legacy-route", nvapi.ComputeDomainCliqueProtocolPersistentAgentV1)
...
func TestWholeCliqueIsolationIncludesStartupIdentityAfterRouteLoss(t *testing.T) {
	f := newNodeAttestationFixture(t, "lost-topology", nvapi.ComputeDomainCliqueProtocolPersistentAgentV1)
	...
	delete(lost.Labels, persistentAgentIsolationLabelKey)
	require.NoError(t, f.manager.nodeInformer.GetStore().Add(lost))
...
func TestSplitWholeCliqueIsolationPublishesNeitherContender(t *testing.T) {
	legacy := newNodeAttestationFixture(t, "controller-a", nvapi.ComputeDomainCliqueProtocolPersistentAgentV1)
	controller := newNodeAttestationFixture(t, "controller-b", nvapi.ComputeDomainCliqueProtocolPersistentAgentV1)
	...
	require.NoError(t, legacy.manager.nodeInformer.GetStore().Add(controller.node.DeepCopy()))
...
func TestRetirementFencesControllerIsolatedNodeAfterRouteLoss(t *testing.T) {
	...
	node := &corev1.Node{ObjectMeta: metav1.ObjectMeta{
		Name: "node-a",
		Labels: map[string]string{
			persistentAgentIsolationLabelKey: cdUID,
		},
		Annotations: map[string]string{
	...
	require.Empty(t, updated.Labels[persistentAgentIsolationLabelKey])
...
func TestNodeAttestationRejectsInexactAuthorization(t *testing.T) {
	tests := map[string]func(*nodeAttestationFixture){
		...
		"config DomainID": func(f *nodeAttestationFixture) {
			...
			config.Protocol = nvapi.ComputeDomainCliqueProtocolPersistentAgentV1
			f.claim.Status.Allocation.Devices.Config[0].Opaque.Parameters.Raw, _ = json.Marshal(config)
		},
		"extra allocation result": func(f *nodeAttestationFixture) {
			f.claim.Status.Allocation.Devices.Results = append(f.claim.Status.Allocation.Devices.Results, f.claim.Status.Allocation.Devices.Results[0])
		},
		...
		"missing channel":    func(f *nodeAttestationFixture) { f.cd.Spec.Channel = nil },
	}
	for name, mutate := range tests {
		t.Run(name, func(t *testing.T) {
			f := newNodeAttestationFixture(t, "reject", nvapi.ComputeDomainCliqueProtocolPersistentAgentV1)
			mutate(f)
			require.NoError(t, f.manager.computeDomainInformer.GetStore().Update(f.cd.DeepCopy()))
