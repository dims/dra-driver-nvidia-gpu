...
func BenchmarkSnapshotHash144(b *testing.B) {
	...
	b.ResetTimer()
	for range b.N {
		if _, err := cdclique.CanonicalHash(members); err != nil {
			b.Fatal(err)
		}
	}
}
		...
		for member := range membersPerClique {
			...
			node := &corev1.Node{ObjectMeta: metav1.ObjectMeta{
				...
				Labels: map[string]string{
					...
					persistentAgentIsolationLabelKey: state.cdUID,
				},
			}}
...
func benchmarkIndexedReconcileInputs(b *testing.B, cliques, membersPerClique int) {
	...
	for range b.N {
		...
		for _, cliqueID := range state.cliqueIDs {
			...
			pods, err := (snapshotDaemonProvider{}).candidatePods(state.pods, state.cdUID, nodes)
			if err != nil {
				b.Fatal(err)
			}
