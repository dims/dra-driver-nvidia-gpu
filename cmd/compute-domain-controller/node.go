...
// after persistent-agent retirement has durably fenced every published daemon.
...
func (m *NodeManager) RemoveComputeDomainLabelsAndAttestations(ctx context.Context, cdUID string, removeAttestation bool) error {
	...
	if removeAttestation {
		...
		// before retirement completes. The persistent-agent isolation label is
		...
		isolatedNodes, listErr := listNodes(persistentAgentIsolationLabelKey)
		if listErr != nil {
	...
	for _, node := range nodesByName {
		...
		// persistent-agent isolation inventory for this ComputeDomain.
		newNode := node.DeepCopy()
		delete(newNode.Labels, computeDomainLabelKey)
		if removeAttestation {
			delete(newNode.Labels, persistentAgentIsolationLabelKey)
