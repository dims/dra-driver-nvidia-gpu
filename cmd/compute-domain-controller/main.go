...
func newApp() *cli.App {
	...
	app := &cli.App{
		...
		Action: func(c *cli.Context) error {
			...
			requiresControllerOwned, err := controllerOwnedStateRequired(c.Context, config)
			if err != nil {
				return err
			}
			if imexConfig.EffectiveHostManaged() && requiresControllerOwned {
				return fmt.Errorf("imex.mode=hostManaged is unsafe while controller-owned state exists; drain and perform the verified fabric recovery procedure first")
			}
			if !flags.leaderElectionConfig.Enabled && requiresControllerOwned {
				return fmt.Errorf("leader election is required while controller-owned state exists")
			}
