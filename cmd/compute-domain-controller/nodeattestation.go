...
func (m *ControllerOwnedCliqueManager) reconcileNodeAttestation(ctx context.Context, nodeName string) error {
	...
	if startupCliqueID != cliqueID || node.Annotations[computeDomainCliqueCapabilityAnnotationKey] != string(nvapi.ComputeDomainCliqueProtocolControllerV1) {
		return m.publishNodeAttestation(ctx, node, nil)
	}
...
func (m *ControllerOwnedCliqueManager) reportWholeCliqueIsolationPending(ctx context.Context, cd *nvapi.ComputeDomain, cliqueID string) {
	...
	message := fmt.Sprintf(
		"waiting for every Node in physical clique %q to carry %s=%s, report controller-v1 capability and matching startup topology, and have no bare or foreign ComputeDomain route",
...
func (m *ControllerOwnedCliqueManager) wholeCliqueIsolated(cliqueID string, cdUID types.UID) (bool, error) {
	...
	for _, object := range objects {
		...
		startupCliqueID := node.Annotations[computeDomainCliqueStartupAnnotationKey]
		if currentCliqueID != cliqueID || startupCliqueID != cliqueID || node.Annotations[computeDomainCliqueCapabilityAnnotationKey] != string(nvapi.ComputeDomainCliqueProtocolControllerV1) {
