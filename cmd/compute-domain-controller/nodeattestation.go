...
func (m *PersistentAgentManager) enqueueWorkloadPod(obj any) {
...
func (m *PersistentAgentManager) enqueueAttestationNodeChange(previous, current any) {
	...
	for cliqueID := range cliques {
		objects, err := m.nodeInformer.GetIndexer().ByIndex(physicalCliqueIndex, cliqueID)
		if err != nil {
			continue
		}
...
func attestationBoundaryChanged(previous, current *corev1.Node) bool {
	...
	for _, key := range []string{gpuCliqueNodeLabelKey, persistentAgentIsolationLabelKey, computeDomainLabelKey} {
...
func (m *PersistentAgentManager) enqueueResourceClaim(obj any) {
...
func (m *PersistentAgentManager) enqueueClaimTemplate(obj any) {
...
func (m *PersistentAgentManager) runAttestationWorker(ctx context.Context) {
	for {
...
func (m *PersistentAgentManager) reconcileNodeAttestation(ctx context.Context, nodeName string) error {
	node, err := m.nodeLister.Get(nodeName)
	if err != nil {
		return nil
	}
	// Once deletion begins, freeze an existing persistent-agent route and its
	// workload identity. The route keeps the exact DaemonSet Pod alive until
	// retirement evidence is durable; the identity lets the retirement path
	...
	slices.SortFunc(candidates, func(a, b attestedNodeCandidate) int {
		...
		if c := cmp.Compare(a.computeDomain.UID, b.computeDomain.UID); c != 0 {
			return c
		}
		return cmp.Compare(a.pod.UID, b.pod.UID)
	})
	if len(candidates) == 0 {
	...
	if startupCliqueID != cliqueID || node.Annotations[computeDomainCliqueCapabilityAnnotationKey] != string(nvapi.ComputeDomainCliqueProtocolPersistentAgentV1) {
		return m.publishNodeAttestation(ctx, node, nil)
	}
	...
	if err := m.reservePhysicalClique(ctx, winner.computeDomain, cliqueID); err != nil {
...
func (m *PersistentAgentManager) clearWholeCliqueIsolationPending(cdUID types.UID, cliqueID string) {
	m.formationWarningsMu.Lock()
	delete(m.formationWarnings, string(cdUID)+"\x00"+cliqueID)
	m.formationWarningsMu.Unlock()
}

func (m *PersistentAgentManager) reportWholeCliqueIsolationPending(ctx context.Context, cd *nvapi.ComputeDomain, cliqueID string) {
	...
	message := fmt.Sprintf(
		"waiting for every Node in physical clique %q to carry %s=%s, report persistent-agent capability and matching startup topology, and have no bare or foreign ComputeDomain route",
		cliqueID, persistentAgentIsolationLabelKey, cd.UID,
	)
...
func (m *PersistentAgentManager) publishedSnapshotRetainsNode(cdUID, nodeUID types.UID, cliqueID string) (bool, error) {
	...
	if cliqueID != "" {
		...
		activated := m.validatedActivations[cdclique.ReservationName(cliqueID)] != ""
...
// reporting the physical clique for the exact persistent-agent ComputeDomain UID
...
func (m *PersistentAgentManager) wholeCliqueIsolated(cliqueID string, cdUID types.UID) (bool, error) {
	objects, err := m.nodeInformer.GetIndexer().ByIndex(physicalCliqueIndex, cliqueID)
	if err != nil {
		return false, err
	}
	...
	for _, object := range objects {
		...
		if !ok || node.Labels[persistentAgentIsolationLabelKey] != string(cdUID) {
			return false, nil
		}
		currentCliqueID := node.Labels[gpuCliqueNodeLabelKey]
		startupCliqueID := node.Annotations[computeDomainCliqueStartupAnnotationKey]
		if currentCliqueID != cliqueID || startupCliqueID != cliqueID || node.Annotations[computeDomainCliqueCapabilityAnnotationKey] != string(nvapi.ComputeDomainCliqueProtocolPersistentAgentV1) {
		...
		if route != "" && (route != string(cdUID) || !validNodeAttestation(node)) {
...
func candidateMatchesNodeAttestation(candidate attestedNodeCandidate, node *corev1.Node) bool {
	...
	return attestation.ComputeDomainUID == candidate.computeDomain.UID &&
		...
		attestation.ResourceClaimUID == candidate.claim.UID &&
		node.Labels[computeDomainLabelKey] == string(candidate.computeDomain.UID)
}

func validNodeAttestation(node *corev1.Node) bool {
	...
	return attestation.ComputeDomainUID != "" && string(attestation.ComputeDomainUID) == node.Labels[computeDomainLabelKey] &&
		node.Labels[persistentAgentIsolationLabelKey] == string(attestation.ComputeDomainUID) &&
		attestation.NodeUID == node.UID && attestation.PodUID != "" && attestation.ResourceClaimUID != ""
}
...
func (m *PersistentAgentManager) liveNodeAttestationAuthorized(node *corev1.Node) bool {
	if !validNodeAttestation(node) {
		return false
	}
...
func (m *PersistentAgentManager) validateAttestationCandidate(pod *corev1.Pod, node *corev1.Node) (attestedNodeCandidate, bool) {
	...
	for i := range pod.Spec.ResourceClaims {
		...
		if !ok || cd.DeletionTimestamp != nil || cd.Namespace != pod.Namespace || cd.Spec.Channel == nil || cd.Spec.Channel.ResourceClaimTemplate.Name != rct.Name {
			continue
		}
		...
		// Persistent-agent mode uses claim-backed attestation.
		...
		return attestedNodeCandidate{computeDomain: cd, pod: pod, claim: claim}, true
	}
	return attestedNodeCandidate{}, false
}
...
func allocatedChannelConfig(claim *resourceapi.ResourceClaim) (*nvapi.ComputeDomainChannelConfig, bool) {
	...
	if len(results) != 1 || results[0].Driver != DriverName || results[0].Request != "channel" {
		return nil, false
	}
...
func (m *PersistentAgentManager) publishNodeAttestation(ctx context.Context, node *corev1.Node, candidate *attestedNodeCandidate) error {
	...
	if candidate == nil {
		...
		// Persistent-agent reconciliation must not erase it merely because there
		// is no persistent-agent claim candidate on this Node. A persistent-agent
		...
		attestation := computeDomainNodeAttestation{
			...
			ResourceClaimUID: candidate.claim.UID,
		}
