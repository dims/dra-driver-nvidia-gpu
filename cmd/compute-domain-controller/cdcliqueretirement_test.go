...
func retirementFixture(t *testing.T, workloadPhase corev1.PodPhase) (*PersistentAgentManager, *nvapi.ComputeDomain, *nvapi.ComputeDomainCliqueSnapshot, *attestationCoreClient) {
	t.Helper()
	...
	snapshot := &nvapi.ComputeDomainCliqueSnapshot{
		...
		Spec: nvapi.ComputeDomainCliqueSnapshotSpec{ComputeDomainUID: cd.UID, CliqueID: "clique", Capacity: 18},
		Status: nvapi.ComputeDomainCliqueSnapshotStatus{
			...
			Members: []nvapi.ComputeDomainCliqueMember{{
				...
				PodUID: types.UID("daemon-pod-uid"), PodIP: "10.0.0.1",
			}},
		},
	}
	attestation := computeDomainNodeAttestation{
		...
		ResourceClaimUID: types.UID("claim-uid"),
	}
	...
	manager := NewPersistentAgentManager(&ManagerConfig{driverNamespace: "driver", clientsets: flags.ClientSets{Core: core, Nvidia: nvidia}})
...
func retirementEvidenceFor(snapshot *nvapi.ComputeDomainCliqueSnapshot, reason string, witnessPodUID types.UID, witnessBootID string) *nvapi.ComputeDomainCliqueRetirementEvidence {
	...
	return &nvapi.ComputeDomainCliqueRetirementEvidence{
		...
		Spec: nvapi.ComputeDomainCliqueRetirementEvidenceSpec{
			...
			Index: member.Index, NodeUID: member.NodeUID,
			ActivationBootID: member.NodeBootID, WitnessBootID: witnessBootID,
			WitnessPodUID: witnessPodUID,
		},
	}
}
...
func TestRetirementEvidenceSurvivesOriginalPodDeletion(t *testing.T) {
	...
	require.NoError(t, err)
	evidence := retirementEvidenceFor(retiring, nvapi.ComputeDomainCliqueRetirementEvidenceReasonProcessExit, types.UID("daemon-pod-uid"), "boot-old")
...
func TestRetirementAcceptsReplacementAfterVerifiedNodeReboot(t *testing.T) {
	...
	evidence := retirementEvidenceFor(retiring, nvapi.ComputeDomainCliqueRetirementEvidenceReasonNodeReboot, types.UID("replacement-uid"), "boot-new")
	_, err = manager.config.clientsets.Nvidia.ResourceV1beta1().ComputeDomainCliqueRetirementEvidences("driver").Create(context.Background(), evidence, metav1.CreateOptions{})
	require.NoError(t, err)
	delete(core.pods, podMapKey("driver", "daemon-pod"))
...
func TestRetirementAcceptsSamePodUIDAfterVerifiedNodeReboot(t *testing.T) {
	...
	require.NoError(t, err)
	core.nodes["node"].Status.NodeInfo.BootID = "boot-new"
	evidence := retirementEvidenceFor(retiring, nvapi.ComputeDomainCliqueRetirementEvidenceReasonNodeReboot, types.UID("daemon-pod-uid"), "boot-new")
	_, err = manager.config.clientsets.Nvidia.ResourceV1beta1().ComputeDomainCliqueRetirementEvidences("driver").Create(context.Background(), evidence, metav1.CreateOptions{})
	require.NoError(t, err)
	delete(core.pods, podMapKey("driver", "daemon-pod"))
...
func TestRetirementRejectsReplacementWithoutBootEpochChange(t *testing.T) {
	...
	retiring, err := manager.config.clientsets.Nvidia.ResourceV1beta1().ComputeDomainCliqueSnapshots("driver").Get(context.Background(), original.Name, metav1.GetOptions{})
	require.NoError(t, err)
	evidence := retirementEvidenceFor(retiring, nvapi.ComputeDomainCliqueRetirementEvidenceReasonNodeReboot, types.UID("replacement-uid"), "boot-old")
	_, err = manager.config.clientsets.Nvidia.ResourceV1beta1().ComputeDomainCliqueRetirementEvidences("driver").Create(context.Background(), evidence, metav1.CreateOptions{})
	require.NoError(t, err)
...
func TestSuccessorReservationClearsFormerActivationMemo(t *testing.T) {
	...
	successor := &nvapi.ComputeDomainCliqueReservation{
		...
		Spec: nvapi.ComputeDomainCliqueReservationSpec{
			CliqueID: "clique", ComputeDomainUID: types.UID("new"),
		},
	}
...
func retirementReservation(cdUID types.UID) *nvapi.ComputeDomainCliqueReservation {
	return &nvapi.ComputeDomainCliqueReservation{
		ObjectMeta: metav1.ObjectMeta{
			Name: cdclique.ReservationName("clique"), UID: types.UID("reservation-uid"),
			Labels: map[string]string{computeDomainLabelKey: string(cdUID)},
		},
		Spec: nvapi.ComputeDomainCliqueReservationSpec{
			CliqueID: "clique", ComputeDomainUID: cdUID,
		},
	}
}
...
func TestDeleteSnapshotsReleasesNeverPublishedReservation(t *testing.T) {
