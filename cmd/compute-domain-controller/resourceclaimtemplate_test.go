...
func TestValidateExistingResourceClaimTemplate(t *testing.T) {
	...
	parameters, err := json.Marshal(nvapi.ComputeDomainDaemonConfig{TypeMeta: metav1.TypeMeta{APIVersion: nvapi.GroupName + "/" + nvapi.Version, Kind: nvapi.ComputeDomainDaemonConfigKind}, DomainID: string(uid), Protocol: nvapi.ComputeDomainCliqueProtocolPersistentAgentV1})
	...
	require.NoError(t, validateExistingResourceClaimTemplate(rct, "driver", rct.Name, uid, computeDomainResourceClaimTemplateTargetDaemon, "daemon", computeDomainDaemonDeviceClass, nvapi.ComputeDomainCliqueProtocolPersistentAgentV1, ""))

	spoof := rct.DeepCopy()
	spoof.Spec.Spec.Devices.Config[0].Opaque.Driver = "attacker.example.com"
	require.ErrorContains(t, validateExistingResourceClaimTemplate(spoof, "driver", spoof.Name, uid, computeDomainResourceClaimTemplateTargetDaemon, "daemon", computeDomainDaemonDeviceClass, nvapi.ComputeDomainCliqueProtocolPersistentAgentV1, ""), "unexpected request")
	...
	require.ErrorContains(t, validateExistingResourceClaimTemplate(spoof, "driver", spoof.Name, uid, computeDomainResourceClaimTemplateTargetDaemon, "daemon", computeDomainDaemonDeviceClass, nvapi.ComputeDomainCliqueProtocolPersistentAgentV1, ""), "decode existing")
}
