...
func (r *persistentAgentRuntime) selectSnapshot(state *PersistentAgentDesiredState) error {
	if state == nil || state.ComputeDomainUID == "" || state.CliqueID != r.cliqueID {
...
func snapshotRuntimeState(state *PersistentAgentDesiredState) string {
...
func runPersistentAgent(ctx context.Context, cancel context.CancelFunc, daemonFlags *Flags, clientsets flags.ClientSets, bootID string) error {
	...
	if err := controller.ClearSnapshotAppliedState(ctx, &PersistentAgentDesiredState{}); err != nil {
	...
	ops := persistentAgentApplyOperations{
		...
		writeAppliedState: func(state *PersistentAgentDesiredState) error {
		...
		clearAppliedState: func(state *PersistentAgentDesiredState) error {
	...
	go func() {
		defer waitGroup.Done()
		if err := runPersistentAgentApplyLoop(ctx, controller, ops); err != nil {
