...
// PersistentAgentDesiredState couples one complete peer map with the exact
...
type PersistentAgentDesiredState struct {
...
type persistentAgentSnapshotIdentity struct {
	uid        types.UID
	generation int64
	hash       string
	retiring   bool
...
func (s *PersistentAgentDesiredState) identity() persistentAgentSnapshotIdentity {
	if s.RetirementEvidence != nil {
		return persistentAgentSnapshotIdentity{
...
// PersistentAgentSnapshotManager is the read-only persistent-agent path. It
// validates that a committed snapshot authorizes this exact Node, Pod and IP
// before exposing the complete peer map to IMEX.
type PersistentAgentSnapshotManager struct {
	...
	currentHash        string
	desired            persistentAgentSnapshotIdentity
	applied            persistentAgentSnapshotIdentity
	retired            persistentAgentSnapshotIdentity
	retirementStarted  bool
	desiredStateChan   chan *PersistentAgentDesiredState
}

func NewPersistentAgentSnapshotManager(config *ManagerConfig) *PersistentAgentSnapshotManager {
...
func (m *PersistentAgentSnapshotManager) Start(ctx context.Context) error {
...
func (m *PersistentAgentSnapshotManager) Stop() error {
	if m.cancel != nil {
		m.cancel()
	}
	m.waitGroup.Wait()
...
func (m *PersistentAgentSnapshotManager) DesiredStateChan() <-chan *PersistentAgentDesiredState {
	return m.desiredStateChan
}

func (m *PersistentAgentSnapshotManager) HasActiveOrRetiringSnapshot() bool {
...
func (m *PersistentAgentSnapshotManager) MarkApplied(state *PersistentAgentDesiredState) {
...
func (m *PersistentAgentSnapshotManager) MarkRetired(state *PersistentAgentDesiredState) {
...
func (m *PersistentAgentSnapshotManager) WriteAppliedState(ctx context.Context, state *PersistentAgentDesiredState) error {
	if state == nil || state.Receipt == nil {
		return nil
	}
...
func (m *PersistentAgentSnapshotManager) ClearAppliedState(ctx context.Context, state *PersistentAgentDesiredState) error {
	if state == nil {
		return nil
	}
...
func (m *PersistentAgentSnapshotManager) patchAppliedAnnotation(ctx context.Context, value string) error {
...
func (m *PersistentAgentSnapshotManager) PublishRetirementEvidence(ctx context.Context, state *PersistentAgentDesiredState) error {
	if state == nil || state.RetirementEvidence == nil {
	...
	validController := controller != nil && controller.APIVersion == "apps/v1" && controller.Kind == "DaemonSet" &&
		controller.Name == "dra-driver-nvidia-gpu-persistent-agent" && controller.UID != ""
	if !validController {
...
func (m *PersistentAgentSnapshotManager) enqueue(obj any) {
	if m.informer.HasSynced() {
		m.reconcilePersistentSnapshots()
	}
}

func (m *PersistentAgentSnapshotManager) reconcilePersistentSnapshots() {
...
func (m *PersistentAgentSnapshotManager) acceptReleasedReservation() {
	m.mu.Lock()
	retired := m.retired
...
func (m *PersistentAgentSnapshotManager) acceptFenced(snapshot *nvapi.ComputeDomainCliqueSnapshot) {
	m.resetRetiredSnapshot(snapshot.UID)
}

func (m *PersistentAgentSnapshotManager) resetRetiredSnapshot(expectedUID types.UID) {
	m.mu.Lock()
	defer m.mu.Unlock()
	...
	m.currentHash = ""
	m.desired = persistentAgentSnapshotIdentity{}
	m.applied = persistentAgentSnapshotIdentity{}
	m.retired = persistentAgentSnapshotIdentity{}
	m.retirementStarted = false
}

func (m *PersistentAgentSnapshotManager) consume(snapshot *nvapi.ComputeDomainCliqueSnapshot) error {
...
func (m *PersistentAgentSnapshotManager) acceptIdentityLocked(identity persistentAgentSnapshotIdentity) error {
...
func (m *PersistentAgentSnapshotManager) validate(snapshot *nvapi.ComputeDomainCliqueSnapshot) (*PersistentAgentDesiredState, error) {
	if snapshot == nil {
	...
	if protocol != nvapi.ComputeDomainCliqueProtocolPersistentAgentV1 {
		return nil, fmt.Errorf("snapshot protocol %q is not persistent-agent-v1", protocol)
	}
	...
	if metav1.GetControllerOf(snapshot) != nil {
	...
	state := &PersistentAgentDesiredState{
