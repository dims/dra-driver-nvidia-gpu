...
func TestPublishRetirementEvidenceIsExactDurableAndIdempotent(t *testing.T) {
	...
	pod := &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			...
			OwnerReferences: []metav1.OwnerReference{{APIVersion: "apps/v1", Kind: "DaemonSet", Name: "dra-driver-nvidia-gpu-persistent-agent", UID: types.UID("daemonset-uid"), Controller: &controller}},
		},
		Spec: corev1.PodSpec{NodeName: "node-a"},
	}
	core := &retirementPodClient{pod: pod}
	nvidia := nvfake.NewSimpleClientset()
	manager := &PersistentAgentSnapshotManager{config: &ManagerConfig{
	...
	state := &PersistentAgentDesiredState{RetirementEvidence: &nvapi.ComputeDomainCliqueRetirementEvidenceSpec{
...
func TestPublishRetirementEvidenceRejectsDifferentWitnessPodUID(t *testing.T) {
	...
	err := manager.PublishRetirementEvidence(context.Background(), &PersistentAgentDesiredState{RetirementEvidence: &nvapi.ComputeDomainCliqueRetirementEvidenceSpec{
...
func TestPersistentAgentAppliedStateUsesGuardedIdempotentJSONPatch(t *testing.T) {
	...
	core := &retirementPodClient{pod: pod}
	manager := &PersistentAgentSnapshotManager{config: &ManagerConfig{
		clientsets: flags.ClientSets{Core: core},
		nodeName:   "node-a", podNamespace: pod.Namespace, podName: pod.Name, podUID: string(pod.UID),
	}}
	state := &PersistentAgentDesiredState{Receipt: &nvapi.ComputeDomainCliqueSnapshotReceipt{
