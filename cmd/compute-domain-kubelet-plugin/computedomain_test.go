...
func TestComputeDomainManagerStartsWithoutSnapshotAPI(t *testing.T) {
	...
	manager.persistentAgentsEnabled = func() bool { return true }
...
func TestComputeDomainManagerLegacyDefaultSkipsControllerReaders(t *testing.T) {
	...
	manager.persistentAgentsEnabled = func() bool { return false }

	ctx, cancel := context.WithCancel(context.Background())
	require.NoError(t, manager.Start(ctx))
	require.False(t, manager.persistentReadersOn)
...
func TestPersistedAgentRequiresStrictFabricErrorsAfterGateDisable(t *testing.T) {
	...
	require.True(t, manager.hasPersistedAgentProtocol())
	manager.persistentReadersOn = manager.persistentAgentsEnabled() || manager.hasPersistedAgentProtocol()

	err := manager.validatePersistentReaderPrerequisites()
	require.ErrorContains(t, err, "persistent-agent state requires feature gate CrashOnNVLinkFabricErrors=true")
}
...
func TestComputeDomainManagerStartsInRetirementRecoveryOnlyMode(t *testing.T) {
	...
	require.True(t, manager.persistentReadersOn)
...
func TestTopologyRecoveryConsumesControllerFenceAndRepublishesIdentity(t *testing.T) {
	...
	require.Equal(t, string(nvapi.ComputeDomainCliqueProtocolPersistentAgentV1), updated.Annotations[computeDomainCliqueCapabilityAnnotationKey])
...
func TestRefreshGPUCliqueIDInvalidatesEmptyToNonempty(t *testing.T) {
	...
	coreClient := &recordingCoreClient{nodes: map[string]*corev1.Node{testNodeName: node}}
	manager := newTestComputeDomainManager(t, nvfake.NewSimpleClientset(), coreClient, "")
	manager.persistentReadersOn = true
	manager.getCliqueIDFunc = func() (string, error) { return "new-clique", nil }
	err := manager.refreshGPUCliqueID(context.Background())
...
func TestPersistentAgentReadinessUsesExactCachedIdentityAndReceipt(t *testing.T) {
	manager, cd, _, pod := readyPersistentAgentManager(t)
...
func TestPersistentAgentReadinessUsesExactSnapshotReservationPodAndReceipt(t *testing.T) {
	manager, cd, snapshot, pod := readyPersistentAgentManager(t)
	cd = cd.DeepCopy()
...
func TestPersistentAgentReadinessLiveDeletionBarrier(t *testing.T) {
	manager, cd, _, _ := readyPersistentAgentManager(t)
...
func TestPersistentAgentReadinessFailsClosedWithoutCliqueOrSnapshotAPI(t *testing.T) {
	manager, cd, _, _ := readyPersistentAgentManager(t)
...
func readyPersistentAgentManager(t *testing.T) (*ComputeDomainManager, *nvapi.ComputeDomain, *nvapi.ComputeDomainCliqueSnapshot, *corev1.Pod) {
	t.Helper()
	...
	snapshot := &nvapi.ComputeDomainCliqueSnapshot{
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				...
				gpuCliqueLabelKey:     manager.CliqueID(),
