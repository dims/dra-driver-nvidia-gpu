...
func TestCliqueAPIActionsSeparateAttemptsFromConfirmedWrites(t *testing.T) {
	...
	const protocol = "persistent-agent-v1"
	...
	for _, expected := range []string{
		`nvidia_dra_cdc_api_actions_total{operation="create",protocol="persistent-agent-v1",resource="reservation",result="success"} 1`,
		`nvidia_dra_cdc_api_actions_total{operation="create",protocol="persistent-agent-v1",resource="reservation",result="already_exists"} 1`,
		`nvidia_dra_cdc_api_actions_total{operation="get",protocol="persistent-agent-v1",resource="reservation",result="success"} 1`,
		`nvidia_dra_cdc_api_actions_total{operation="write_barrier_get",protocol="persistent-agent-v1",resource="snapshot",result="success"} 1`,
		`nvidia_dra_cdc_api_actions_total{operation="status_update",protocol="persistent-agent-v1",resource="snapshot",result="error"} 1`,
		`nvidia_dra_cdc_api_actions_total{operation="status_update",protocol="persistent-agent-v1",resource="snapshot",result="forbidden"} 1`,
		`nvidia_dra_cdc_api_actions_total{operation="status_update",protocol="persistent-agent-v1",resource="snapshot",result="timeout"} 1`,
		...
		`nvidia_dra_cdc_api_writes_total{operation="finalizer_remove",protocol="persistent-agent-v1",resource="snapshot"} 1`,
