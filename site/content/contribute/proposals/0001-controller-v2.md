---
title: 0001 — Controller-v2 warm ComputeDomain daemons
linkTitle: Controller-v2 warm daemons
weight: 1
...
Implement `controller-v2` as one narrow change to the verified
`controller-v1` design: replace the per-ComputeDomain daemon DaemonSet and its
per-Pod ResourceClaims with one installation-scoped warm-agent DaemonSet.
...
machine. Do not require an unreleased Kubernetes version. If the warm-agent
change does not materially improve measured convergence, stop rather than
...
Controller-v2 keeps one helper Pod ready on every capable Node. It is still a
normal Kubernetes DaemonSet, but it is created when the feature is installed,
not once per ComputeDomain. When a controller-v2 snapshot becomes Active, the
existing helper starts the same `nvidia-imex` child that a per-domain daemon Pod
would have started. When the ComputeDomain retires, the helper stops and reaps
that child, writes the existing retirement evidence, and becomes idle again.
...
flowchart LR
    ...
    AGENT[One warm agent per capable Node]
...
Add an immutable `controller-v2` protocol under a new Alpha feature gate.
Controller-v2 uses the controller-v1 reservation, snapshot, allocation,
attestation, receipt, and retirement contracts without copying them. A single
installation-scoped warm-agent DaemonSet consumes controller-v2 snapshots and
supervises at most one IMEX child per Node. The existing physical-clique
reservation guarantees that a warm agent cannot be assigned two active
...
Controller-v2 addresses only the part the driver can remove portably: the
second, per-ComputeDomain daemon scheduling wave. Kubernetes scheduler
improvements may reduce the workload-claim portion in future releases, but v2
must work and be measured on the driver's released supported versions.
...
- Keep legacy and controller-v1 behavior unchanged when v2 is disabled.
...
- Support the DRA API version selected by the existing compatibility layer on
  every Kubernetes minor included in the v2 test matrix.
...
- Making host-managed IMEX and controller-v2 the same mode.

## Why this belongs in the NVIDIA DRA driver

The cost being removed is created by the driver's own per-ComputeDomain daemon
objects. The warm agent also needs NVIDIA-specific management-device access,
IMEX configuration, peer-map installation, READY probing, and retirement
evidence. Kubernetes DRA and the scheduler should continue to handle ordinary
workload claims; they should not own this NVIDIA-specific child process.
...
4. Add no per-ComputeDomain daemon workload object for controller-v2.
...
6. Represent the warm mode with additive protocol/config fields, not copied
   APIs.
7. Localize protocol branching to protocol selection, daemon-provider lookup,
   warm-agent execution, status observation, and cleanup.
8. Prefer deleting a controller-v1-only assumption over adding a parallel v2
   implementation.
...
Controller-v2 selects the warm-agent provider. Reservation, allocation,
...
Add `controller-v2` to `ComputeDomainCliqueProtocol`. As with controller-v1:
...
- disabling the feature gate stops new selection but not reconciliation of
  persisted v2 objects; and
- old binaries reject the unknown protocol instead of treating it as legacy.

Use one Alpha gate, tentatively `WarmComputeDomainDaemons`, and the existing
canary namespace allowlist. Gate-off installations create no warm-agent
objects and start no v2-only readers.

The user-facing request remains one annotation; v2 adds no placement fields:
...
metadata:
  ...
  annotations:
    resource.nvidia.com/requestedComputeDomainCliqueProtocol: controller-v2
spec:
  numNodes: 18
  channel:
...
persist `controller-v2`; an annotation alone never authorizes it.

### Reused API objects

Controller-v2 uses the existing three CRDs:

| Object | Controller-v2 use |
|--------|-------------------|
| `ComputeDomainCliqueReservation` | Same API-server-atomic physical-clique ownership and release record |
| `ComputeDomainCliqueSnapshot` | Same deterministic assignments, exact warm-agent Pod identities, desired peer map, phase, generation, and hash |
...
Do not add a topology inventory API. V2 retains the controller-v1 requirement
that `spec.numNodes`, claim-backed attestation, current clique labels, immutable
startup identity, and the whole-clique isolation label all agree before first
publication. Missing or ambiguous Nodes keep the snapshot Pending.

### Warm-agent fleet

Render one DaemonSet and one ResourceClaimTemplate when the v2 gate is enabled.
The DaemonSet:
...
- runs `compute-domain-daemon run --warm`;
...
`mode: WarmAgent` value. The default remains `PerDomain`. In warm mode the
kubelet plugin validates the installation namespace and protocol, mounts the
existing ComputeDomain state root rather than one domain directory, and
injects the same IMEX management device. It does not look up a synthetic
ComputeDomain.
...
For controller-v2, step 7 chooses the warm-agent Pod on each Node. No
per-ComputeDomain DaemonSet or daemon ResourceClaimTemplate is created. The
snapshot member's existing `PodName`, `PodUID`, `PodIP`, Node UID, boot ID, and
index fields describe that warm-agent incarnation without a schema change.

The v2 snapshot has no shared DaemonSet owner reference. It is retained by its
existing fence finalizer and validated against the ComputeDomain UID,
reservation, protocol, canonical name, and the fixed installation-scoped warm
DaemonSet. Deleting the warm DaemonSet must not cause garbage collection of
active snapshots.

### Warm-agent apply loop
...
by its clique and `protocol=controller-v2`.
...
The controller-v2 kubelet path is the existing controller-v1 path with the
protocol enum extended. It requires:
...
- the exact local Node and warm-agent Pod member;
- warm-agent Pod Ready;
...
snapshot is Active and every exact warm-agent Pod is Ready and reports the
...
2. Active snapshots move to Retiring without changing their member set, hash,
   or generation.
3. Each original warm agent stops and reaps its IMEX child and creates
...
7. The warm-agent Pod remains and can serve the next ComputeDomain.

The controller skips per-domain daemon DaemonSet and daemon template deletion
for v2. It never deletes or rolls the installation-scoped warm fleet as part of
ComputeDomain cleanup.

### Agent upgrades

`OnDelete` is mandatory. A chart upgrade may update the DaemonSet template but
must not replace a warm-agent Pod serving an Active or Retiring snapshot.
...
The Alpha does not implement automatic warm-fleet decommission after v2 has
ever been used. Gate disable stops new v2 selection but leaves the fleet and
reconciliation available until an explicit, future empty-state decommission
protocol exists.

### Informers and queues

The controller reuses the controller-v1 informers and snapshot queue. The warm
provider adds only an index over the existing driver-namespace Pod informer by
Node name. If the current server-side selector cannot include both per-domain
and warm Pods, widen that one namespace-scoped informer and filter its handlers;
do not add a second Pod watch. No new cluster-wide Pod, Node, ResourceClaim, or
template informer is allowed.

The warm agent has one namespace-scoped, clique-filtered snapshot watch and
exact-name reservation reads. It must not watch all cluster reservations or all
ComputeDomains.
...
- only the controller may request/persist v2 and write v2 snapshots;
- only the fixed warm-agent ServiceAccount may read its clique snapshots;
- the warm agent may patch only its own applied annotation;
- Pod-bound identity must match the warm-agent Pod UID and Node name;
- retirement evidence remains create-only and immutable;
- the warm DaemonSet, template, ServiceAccount, and bindings use the immutable
...
The v2 implementation must use the existing version-converting DRA client and
the chart-selected ResourceClaimTemplate API version. New code may not import a
single resource API version as an architectural shortcut.

Run legacy regression tests on every supported minor. Run v2 on every supported
minor where the Phase 0 bootstrap-claim spike passes and document any narrower
Alpha floor explicitly. Kubernetes v1.37 is not a prerequisite; after it is
released and supported, add it as another comparative row.

## Expected request shape

For each ComputeDomain, controller-v2 removes:
...
- one warm-agent applied-annotation patch per selected Node.

The applied annotation is an additional explicit write. The value of v2 comes
from deleting the substantially larger daemon object/scheduling wave, not from
claiming that all controller writes disappeared.
...
Build a small spike, without accepting v2 ComputeDomains, which proves:

1. one installation-scoped ResourceClaimTemplate can prepare one warm Pod per
   capable Node;
...
3. an idle warm Pod can start, READY-check, stop, and reap one test IMEX child;
...
- Add `WarmAgent` mode to the existing daemon opaque config.
- Add the feature gate and explicit request validation.
- Render the warm ResourceClaimTemplate and OnDelete DaemonSet only when the
  gate or persisted-v2 state requires them.
...
- Add the indexed warm-agent provider for controller-v2.
- Skip daemon template and DaemonSet creation/deletion only for v2.
- Publish v2 snapshots through the existing allocator and write barrier.
...
- Parameterize the existing action/write metrics by effective protocol rather
  than copying metric helpers for v2.

Exit criteria:

- the fake-API harness proves v2 creates zero per-domain daemon workload
  objects;
- controller-v1 action/write counts are unchanged;
- v2 uses the same reservation and snapshot transition tests; and
...
- Accept the v2 protocol in the existing snapshot/receipt validator.
- Resolve the exact warm member rather than a per-domain daemon member.
...
- no v2-specific readiness bypass;
...
Run identical controller-v1 and controller-v2 workloads with the same images,
...
- tuned legacy-v1;
- controller-v1;
- controller-v2 with a warm image;
- controller-v2 after an idle-agent restart; and
- host-managed IMEX as a separately labeled reference where supported.

Promotion requires:

- zero per-domain daemon Pods and claims in v2;
...
If v2 does not meet the scale threshold, keep controller-v1 and remove the v2
implementation. Do not compensate by adding speculative APIs.
...
- Warm-config validation and installation-namespace binding.
- Exactly one warm Pod per selected Node; duplicate and stale Pod rejection.
...
- Zero/one/multiple snapshot behavior in the warm agent.
...
- Gate disable with persisted v2 state.
...
- warm Pod disappearance before Active;
- warm Pod disappearance after Active;
- two ComputeDomains contending for one clique;
- legacy/controller-v1/controller-v2 cross-protocol exclusion;
- workload deletion and ComputeDomain deletion ordering; and
- exact clique reuse after verified fence.

For controller-v2, the harness asserts no action creates a per-domain daemon
...
- Warm ServiceAccount cannot mutate Nodes, snapshots, or reservations.
- Warm Pod can patch only its own applied annotation.
- Evidence creation is Pod/Node bound and immutable.
- OnDelete rollout leaves active agent Pods untouched.
- Gate disable keeps persisted v2 reconciliation alive.
...
Ti = warm agent starts IMEX child
Tr = warm agent observes READY and writes receipt
...
- API-server, scheduler, controller, kubelet-plugin, and warm-agent CPU/memory;
- daemon Pod and claim counts; and
- installation-time warm-fleet rollout separately from per-workload latency.

Do not attribute v2 improvement to prewarming unless both installation cost and
...
- Enabling the gate may stage idle warm agents before any namespace is allowed
  to request v2.
- Disabling the gate rejects new v2 requests but continues all persisted v2
  reconciliation.
- Controller-v1 and v2 share reservations, so they cannot overlap on one
  physical clique.
- Rollback to a binary which does not understand v2 is unsupported while any
  v2 ComputeDomain, snapshot, reservation, evidence, applied annotation, or
  possibly-live local child state exists.
- The Alpha does not automatically remove the warm fleet after use.
...
| Warm claim bootstraps through the same DRA driver | Phase 0 proves it before API work; separate DaemonSet starts after kubelet plugin |
...
| Post-scheduling path retains workload scheduler tail | Measure it explicitly; do not claim v2 removes it |
| Same-boot warm Pod loss blocks reuse | Preserve strict quarantine; planned rollout retires first |
...
submission. Defer it until the warm-agent experiment shows the remaining
...
Rejected for the first v2. Snapshot, reservation, receipt, and retirement
...
fallbacks for a failed warm-claim spike.

### Host-managed IMEX

Keep as a separate supported comparison. It changes operational ownership and
isolation semantics; controller-v2 keeps driver-managed per-ComputeDomain IMEX
children.
...
- Every capable Node runs one additional idle Pod and holds one installation
  claim while v2 is staged.
- Warm-agent Pods require careful, slower rollout than ordinary stateless
  DaemonSets.
- V2 retains controller-v1's Node attestation writes and cluster-wide workload
  claim caches.
...
- The first v2 does not solve the ordinary workload-claim scheduler tail.

## Approval requested

Approve these boundaries before implementation:

1. Controller-v2 is controller-v1 plus an installation-scoped warm daemon
...
5. Warm-agent rollout is OnDelete and requires retirement before replacement.
6. Phase 0 may terminate the work if the generic warm claim is not portable.
...
- the existing daemon config's `WarmAgent` mode;
...
It does not add the `controller-v2` protocol or change any ComputeDomain
