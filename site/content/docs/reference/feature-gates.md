...
| `ControllerOwnedCDCliques` | Alpha | `false` | Allows a newly created ComputeDomain carrying `resource.nvidia.com/requestedComputeDomainCliqueProtocol: controller-v1` to select the controller-owned, single-writer `ComputeDomainCliqueSnapshot` protocol. Unmarked and existing ComputeDomains remain legacy. Requires `ComputeDomainCliques`, `IMEXDaemonsWithDNSNames`, `CrashOnNVLinkFabricErrors`, kubelet clique-label publication, controller leader election, an operator-controlled canary namespace, and exclusive use of a new or externally quiesced/reset physical clique. Alpha supports one chart installation and the primary driver namespace only. |
...
| `ControllerOwnedCDCliques` | `ComputeDomainCliques`, `IMEXDaemonsWithDNSNames`, `CrashOnNVLinkFabricErrors` |
