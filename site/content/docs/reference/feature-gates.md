...
| `PersistentComputeDomainAgents` | Alpha | `false` | Replaces per-ComputeDomain IMEX daemon Pods with one reusable agent per capable Node for the entire installation. Requires Kubernetes v1.34+, `ComputeDomainCliques`, `IMEXDaemonsWithDNSNames`, `CrashOnNVLinkFabricErrors`, kubelet clique-label publication, controller leader election, and the persistent-agent admission boundary. Retire every legacy ComputeDomain before enabling it. |
...
| `PersistentComputeDomainAgents` | `ComputeDomainCliques`, `IMEXDaemonsWithDNSNames`, `CrashOnNVLinkFabricErrors` |
